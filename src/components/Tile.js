...
class Tile extends Component {
    ...
    render() {
        return (
            <div className={this.props.type==='main' ? Style.nonExpandingContainerMajor : Style.nonExpandingContainerMinor} 
                onClick={this.viewShow}>
                    <div className={this.props.type==='main' ? Style.flexGridWholeRow : Style.flexGridHalves}
                    style={{
                        backgroundImage:`url(${this.props.show.poster_src})`,
                        backgroundRepeat: 'no-repeat'
                    }}>
                    <h3 className={Style.title} onClick={this.viewShow}>
                        {this.props.showType==="movie" ? this.props.show.title: this.props.show.name}
                    </h3>
                    </div>
                </div>
        )
